/**
 * Script to create a test user with referrals and commissions
 *
 * Usage:
 * npx tsx scripts/create-test-user-with-referrals.ts
 */
...
async function createTestUserWithReferrals() {
  console.log(`\n🚀 Creating test user with referrals...`)

  const mainUserEmail = "gregory.test@mvppir.com"
  const mainUserName = "Gregory Test"
  ...
  if (existingUser) {
    console.log(`\n✅ Using existing user: ${existingUser.name} (${existingUser.email})`)
    mainUser = existingUser
  } else {
    // Generate wallet for main user
    ...
    mainUser = await prisma.user.create({
      data: {
        ...
        blockedBalance: 100, // $100 blocked to be RECRUIT
        totalDirects: 0,
        lifetimeVolume: 0,
      },
    })

    console.log(`\n✅ Main user created: ${mainUser.name} (${mainUser.email})`)
    console.log(`   ID: ${mainUser.id}`)
    console.log(`   Referral Code: ${mainUser.referralCode}`)

    // Create deposit address for main user
    ...
    await prisma.walletTransaction.create({
      data: {
        ...
        tokenDecimals: 6,
        amount: 100,
        rawAmount: "100000000", // 100 USDC (6 decimals)
        type: "CREDIT",
        status: "CONFIRMED",
      },
    })
    ...
    console.log(`   ✅ Initial deposit created: $100`)
  }
  ...
  for (const referral of referrals) {
    ...
    const newUser = await prisma.user.create({
      data: {
        ...
        rankStatus: "ACTIVE",
        referralCode,
        referrerId: mainUser.id, // Link to main user
    ...
    await prisma.commission.create({
      data: {
        userId: mainUser.id, // Main user receives the commission
  ...
  console.log(`   (Use Better Auth to set a password or login via magic link)`)
}

// Run script
createTestUserWithReferrals()
  .then(() => process.exit(0))
  .catch((error) => {
